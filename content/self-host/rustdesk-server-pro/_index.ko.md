...
- 계정
- [Web console](/docs/ko/self-host/rustdesk-server-pro/console/)
...
- [Multiple relay servers](/docs/ko/self-host/rustdesk-server-pro/relay/)(가장 가까운 릴레이를 자동으로 선택함)
- [Custom client generator](/docs/ko/self-host/client-configuration/#1-사용자-지정-클라이언트-생성기-pro-전용-기본-플랜-또는-사용자-지정-플랜)
- 웹소켓
- 웹 클라이언트 자체 호스팅

{{% notice note %}}
집이나 사무실에서 자체 서버를 구축하고 공개 IP/도메인을 통해 연결할 수 없는 경우, [this article](/docs/ko/self-host/nat-loopback-issues/)를 확인해 주세요.
{{% /notice %}}

{{% notice note %}}
계속하기 전에 먼저 이 내용을 읽어보시는 것을 권장합니다, [How does self-hosted server work?](/docs/ko/self-host/#자체-호스팅-서버는-어떻게-작동하나요).
{{% /notice %}}
...
라이선스는 https://rustdesk.com/pricing.html에서 받을 수 있으며, 자세한 내용은 [license](/docs/ko/self-host/rustdesk-server-pro/license/) 페이지를 확인하십시오.
...
이 포트에 대한 자세한 정보는 [here](/docs/ko/self-host/rustdesk-server-oss/install/#방화벽)를 확인하십시오.

### 3. 라이선스 설정

`http://<server ip>:21114`, [log in](/docs/ko/self-host/rustdesk-server-pro/console/#로그인)에 기본 자격 증명 admin/test1234를 사용해 웹 콘솔을 엽니다. `admin`/`test1234`도 동일합니다. [this guide](/docs/ko/self-host/rustdesk-server-pro/license/#라이선스-설정)를 따라 라이선스를 설정하십시오.

### 4. 웹 콘솔용 HTTPS 설정

> 체험 기간 동안 HTTPS를 사용하지 않으려면 이 단계를 건너뛸 수 있지만, HTTPS를 설정한 후 클라이언트의 API 주소를 변경하는 것을 잊지 마세요.

다음은 [manual HTTPS setup](/docs/ko/self-host/rustdesk-server-pro/faq/#웹-콘솔용-https-수동-설정)에 대한 간단한 자습서입니다.

### 5. 자체 호스팅 서버를 사용하도록 클라이언트 구성하기

https://rustdesk.com/docs/ko/self-host/client-configuration/

### 6. WebSocket 설정하기

웹 클라이언트나 [desktop / mobile client](/docs/ko/self-host/client-configuration/advanced-settings/#웹소켓-사용)가 WebSocket을 통해 제대로 작동하려면 리버스 프록시 구성에 다음 설정을 추가해야 합니다.

[WebSocket Secure (WSS) 지원 추가](/docs/ko/self-host/rustdesk-server-pro/faq/#8-id-서버와-릴레이-서버에-websocket-secure-wss-지원을-추가하여-모든-플랫폼에서-안전한-통신을-가능하게-하십시오)

## 서버 업그레이드

이 [guide](/docs/ko/self-host/rustdesk-server-pro/faq/#rustdesk-server-pro의-새-버전이-출시되었습니다-어떻게-업그레이드할-수-있나요)는 낮은 버전에서 RustDesk Server Pro로 업그레이드하는 방법을 다루며, 다양한 설치 방식을 설명합니다.
...
이 [guide](/docs/ko/self-host/rustdesk-server-pro/license/#청구서-라이선스-검색-및-마이그레이션)를 따르십시오.

## 라이선스 업그레이드

[this guide](/docs/ko/self-host/rustdesk-server-pro/license/#라이선스-갱신업그레이드)를 따라 언제든지 더 많은 사용자와 장치를 위한 라이선스를 업그레이드하십시오.
