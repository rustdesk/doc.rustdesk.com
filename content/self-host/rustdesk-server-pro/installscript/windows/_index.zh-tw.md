...
- 舊的 GUI 安裝器已不再維護。
- 如果要長期穩定執行，盡量遷移到 Linux 或 Docker。
...
在 Windows 上執行 rustdesk 需要 Microsoft Visual C++ Redistributable。您可以在[這裡](https://learn.microsoft.com/en-us/cpp/windows/latest-supported-vc-redist)下載。
...
3. 解壓 Windows 安裝程式。
4. 執行安裝程式並按照螢幕上的步驟進行。或者手動安裝 [PM2 或 NSSM](https://rustdesk.com/docs/en/self-host/rustdesk-server-oss/windows/)。
...
1. 打開 IIS（或安裝它）。
2. 為 RustDesk 建立一個新網站，設定繫定（理想情況下為 443）和相關證書。基本設定應該指向一個空白資料夾。（如果您使用預設站點，請確保資料夾中沒有其他檔案）。
...
您可能需要將 SSL 設定更改為 “Require SSL → Ignore”。
