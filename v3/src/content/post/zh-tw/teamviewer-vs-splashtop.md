...
Splashtop 其實已經把一個自架選項——On-Prem——擺上了檯面，因此對於需要由客戶自行營運中介的團隊而言，真正的選擇是要執行誰家的伺服器，而不是要不要執行伺服器。一個完全自架的開放原始碼替代方案，同樣值得被列入同一張評估表，並且應以控制權、工作量與總成本來衡量，而不是以每月的 SaaS 標價來衡量。
